...
Objective: Use 2013 data from the FAO Food Balances and Commodity Balances data to determine the proportion of crops being used as feed for which most of the crop is going to the processing category. Method is inspired by Schwarzmueller et al. (2022).
...
Example of calculations using soy:
...
```{r}
fb <- read_csv(paste0(FAO_path, "food_balance_sheets/FoodBalanceSheetsHistoric_E_All_Data_(Normalized).csv")) %>% 
  filter(Year == 2013)
...
# remove China (We will use China mainland)
fb <- fb %>% 
  filter(!Area == "China")
```
...
We need to add in missing countries that are missing from FB and CB data. We will use a combination of FAO country names and additional countries from fisheries trade data. Not all of these countries will have trade or feed of these animals but they will be included in case there is.
```{r}
...
# check to make sure this is Turkey
soy[152,1]          
soy[152,1] <- "Turkey"
# check to make sure this is Côte d'Ivoire
soy[37,1]                     
soy[37,1] <- "Côte d'Ivoire"
# check to make sure this is Taiwan
soy[33,1]                     
soy[33,1] <- "Taiwan"
...
soy_gap_filled <- soy_gap_filled %>% 
  ...
  mutate(soy_gf = case_when(soy_prop_feed == 0 ~ TRUE,
                                      is.na(soy_prop_feed) ~ TRUE,
                                      is.infinite(soy_prop_feed) ~ TRUE,
                                      TRUE ~ FALSE))

# make a dataframe for supplementary materials
gapfill_df_for_supplementary <- soy_gap_filled %>% 
  filter(!is.na(iso3)) %>% 
  select(Area, iso3, region, soy_prop_feed, soy_prop_feed_gf, soy_gf)

soy_gap_filled <- soy_gap_filled %>% 
  # select only the columns we need for future calculations
...
pk[109,1]
pk[109,1] <- "Turkey"
# check that this is Côte d'Ivoire
pk[27,1]                     
pk[27,1] <- "Côte d'Ivoire"
...
pk_region <- pk_region %>% 
  select(-c(subregion, intermed_region)) %>% 
  # reorder columns
  select(Area, iso3, region, pk_dsq, pk_feed, pk_cake, pk_prop_feed) 
...
pk_gap_filled <- pk_gap_filled %>% 
  ...
  mutate(pk_prop_feed_gf = case_when(pk_prop_feed_gf == 0 ~ pk_prop_feed_gf[Area == continent],
                                     ...
                                     TRUE ~ pk_prop_feed_gf)) %>% 
  # add column to note gapfill YES if gapfilled, FALSE if not
  mutate(pk_gf = case_when(pk_prop_feed == 0 ~ TRUE,
                           is.na(pk_prop_feed) ~ TRUE,
                           is.infinite(pk_prop_feed) ~ TRUE,
                           TRUE ~ FALSE)) %>% 
  ungroup()
...
gapfill_df_for_supplementary <- gapfill_df_for_supplementary %>% 
  full_join(pk_gap_filled %>% 
              filter(!is.na(iso3)) %>% 
              select(iso3, pk_prop_feed, pk_prop_feed_gf, pk_gf)) 
...
oils[131,1] <- "Turkey"
oils[184,1]                     # check to make sure this is Côte d'Ivoire
oils[184,1] <- "Côte d'Ivoire"
...
oil_gap_filled <- oil_gap_filled %>% 
  ...
  # add column to note gapfill YES if gapfilled, FALSE if not
  mutate(oil_gf = case_when(oil_prop_feed == 0 ~ TRUE,
                           is.na(oil_prop_feed) ~ TRUE,
                           is.infinite(oil_prop_feed) ~ TRUE,
                           TRUE ~ FALSE)) %>% 
  ungroup()
  
# make a dataframe for supplementary materials
gapfill_df_for_supplementary <- gapfill_df_for_supplementary %>% 
  full_join(oil_gap_filled %>% 
              filter(!is.na(iso3)) %>% 
              select(iso3, oil_prop_feed, oil_prop_feed_gf, oil_gf)) 
...
Save the new data and the data frame for supplementary
```{r}
# save csv
write_csv(oil_gap_filled, paste0(save_path, "perc_dsq_oilcrops_used_as_feed_gapfilled.csv"))

# save csv
write_csv(gapfill_df_for_supplementary, paste0(save_path, "gapfill_df_for_supplementary.csv"))
```