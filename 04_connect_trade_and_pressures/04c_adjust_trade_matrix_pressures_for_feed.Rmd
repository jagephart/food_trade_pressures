...
feed_crops_item_proportions <- feed_crops %>% 
  ...
  ungroup() %>% 
  # now calculate the proportion of each item contributes to the feed category traded to the consuming country
  group_by(consumer_country, consumer_iso3c, producer_country, producer_iso3c, item_code, item, SPAM_super) %>% 
  summarise(traded_tonnes_proportion = traded_tonnes/feed_cat_traded_tonnes,
            water_proportion = water_total/feed_cat_water_total,
            ghg_proportion = ghg_total/feed_cat_ghg_total,
            disturbance_proportion = disturbance_total/feed_cat_disturbance_total,
            nutrient_proportion = nutrient_total/feed_cat_nutrient_total) %>% 
  ungroup()
  
```

## I think I need to go back and fix gapfilling before continuing. This seems like a good place to stop. Good progress this week but still plenty to do.
