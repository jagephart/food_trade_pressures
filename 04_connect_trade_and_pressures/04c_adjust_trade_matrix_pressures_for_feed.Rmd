...
# Uncomment to see if proportions all add up to 1
...
Combine dfs - add variables for animal system and feed proportions for fish meal to the trade matrix with pressures df (fofm producer and fofm consumer trade matrix). By using a left join we may lose data where we have animal proportions for fish meal but no trade data of fish meal into that country. We are not gapfilling trade data of products so this is just note for methods.
```{r}
...
# left join because this will not discount animal trade 
# but simply directing the feed to the animals it was fed to
...
Note: The below data check explores if there is a difference between the total pressures and tonnage traded before joining with the animal proportions compared to after.
```{r}
...
((sum(feed_crops_used_for_feed$water_pressure_feed) - sum(crop_feed_pressures_adjusted$water_traded_system_feed))/sum(feed_crops_used_for_feed$water_pressure_feed))*100 # -1.675025e-14% we will ignore this as it is likely a rounding error from R
...
# order = producer, consumer, item, food_group, SPAM_super, tonnes traded, ghg, water, nutrient, disturbance

## crop items not used for feed
non_feed_crops_trade_pressures_for_final <- non_feed_crops_joined %>% 
  select(producer_iso3c, consumer_iso3c, item, food_group, SPAM_super,
...
fofm_animal_feed_pressures_trade_for_final <- fofm_animal_feed_pressures_trade_totals %>% 
  # add in SPAM_super column
  mutate(SPAM_super = NA,
         # make item column
         item = paste0(feed_category, "_for_", animal_product),
...
animal_feed_pressures_trade_for_final <- animal_feed_pressures_trade_totals %>% 
  # add in SPAM_super column
  mutate(SPAM_super = "feed",
         item = paste0(feed_category, "_feed_for_", animal_product),
...
# Some of the proportion calculations made zeros,
# filter all trade that is zero tonnes
trade_matrix_w_pressures_accounting_for_feed <- trade_matrix_w_pressures_accounting_for_feed %>% 
  filter(tonnes_traded > 0)
```
