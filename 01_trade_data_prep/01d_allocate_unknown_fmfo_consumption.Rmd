...
The FAO commodity code for the fish oil and fishmeal products we are interested in start with "081.42". We will filter the commodity list to only include products with this code at the begining of the code.
...
to get liveweight equivalent multiply products weight by 2.975207 and is from IFFO, 2009 https://essopenarchive.org/doi/full/10.22541/essoar.167590829.99780929
OR from Jackson, A. Fish in-Fish out. Ratios Explain. 34, (2009).
```{r}
fm_rate <- .225
fo_rate <- .05

1/(fm_rate+fo_rate) * (fm_rate/(fm_rate+fo_rate))
```
...
imports_production <- full_join(x = production_average_converted, y = fmfo_imports,
                                by = c("ISO3_Code" = "consumer_iso3c"))

# make NAs equal to zero and remove country name column and calculate proportion
imports_production <- imports_production %>% 
  select(-country_name) %>% 
...
import_proportion <- imports_production %>% 
  mutate(total_tonnes_supply = import_live_weight_t + production_live_weight_t,
         proportion_imported = import_live_weight_t/total_tonnes_supply)
