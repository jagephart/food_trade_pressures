...
We need to use 2013 data to determine the proportions of these crops because after 2013 FAO swtiched their data methods. After 2013 there is no breakdown of how much of these crops was going towards feed vs other uses.
...
soy[152,1] <- "Turkey"
soy[37,1]                     # check to make sure this is Côte d'Ivoire
...
Melanesia, Micronesia, and Polynesia regions have NA for their soy_prop_feed. To gapfill these regions we will use the Small Island Developing States average. One could also use Oceania average, however this is includes data from Australia and New Zealand which are different economies than these regions that have no data.
...
soy_gap_filled <- soy_gap_filled %>% 
  ...
  mutate(soy_prop_feed_gf = if_else(condition = region %in% c("Melanesia", "Micronesia", "Polynesia"),
                                    true = soy_gap_filled$soy_prop_feed[soy_gap_filled$Area == "Small Island Developing States"],
...
Read in previously made `oils` of percent feed for soy, add region column, and gapfill.
```{r}
# have to rename Turkey because it got a ? symbol added when saved as csv previously
# can't use %in% or == to identify it because it isn't showing up as a string
...
Melanesia, Micronesia, and Polynesia regions have zero for their oil_prop_feed. To gapfill these regions we will use the Small Island Developing States average. One could also use Oceania average, however this is includes data from Australia and New Zealand which are different economies than these regions that have no data.
...
oil_gap_filled <- oil_gap_filled %>% 
  # use "small Island Developing States" average for missing regions 
  mutate(soy_prop_feed_gf = if_else(condition = region %in% c("Melanesia", "Micronesia", "Polynesia"),
                                    true = oil_gap_filled$oil_prop_feed[oil_gap_filled$Area == "Small Island Developing States"],
