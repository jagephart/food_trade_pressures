...
2017 trade data - gapfill imports with export data
```{r}
...
fao_trade_quantity_per_year <- function(fao_trade_matrix_raw, year) {
...
```{r}
# subset columns to match Kastner data columns and subset to only 2017
trade_matrix_2017 <- fao_trade_matrix_raw %>% 
  select(-c(`Reporter Country Code (M49)`, `Partner Country Code (M49)`, `Item Code (CPC)`)) %>% 
  filter(Year == 2017)
...
length(which(is.na(trade_df_2017$Value))) #15523

# save df - can take out NAs later
write_csv(trade_df_2017, paste0(save_path,"Trade_2017_DetailedTradeMatrix_Gapfilled_7_14_22_formatted.csv"))
```
...
h_production_crops <- fread(paste0(fao_data_path, "Production_Crops_2017_E_All_Data_(Normalized)_7_20_22.csv"))
# format data
...
write_csv(h_production_crops_1, paste0(save_path, "Production_Crops_2017_E_All_Data_(Normalized)_7_20_22_formatted.csv"))

# read in recently downloaded production crops processed data from FAO website
h_production_crops_processed <- fread(paste0(fao_data_path, "Production_CropsProcessed_2017_E_All_Data_(Normalized)_7_20_22.csv"))
# format data
...
write_csv(h_production_crops_processed_1, paste0(save_path, "Production_CropsProcessed_2017_E_All_Data_(Normalized)_7_20_22_formatted.csv"))


#read in recently downloaded production livestock data from FAO website
h_production_livestock_primary <- fread(paste0(fao_data_path, "Production_LivestockPrimary_2017_E_All_Data_(Normalized)_7_20_22.csv"))
# format data
...
write_csv(h_production_livestock_primary_1, paste0(save_path, "Production_LivestockPrimary_2017_E_All_Data_(Normalized)_7_20_22_formatted.csv"))
```
...
h_comm_bal_crops <- fread(paste0(fao_data_path, "CommodityBalances_Crops_2017_E_All_Data_(Normalized)_7_20_22.csv"))
...
h_comm_bal_crops_2 <- h_comm_bal_crops_1 %>% 
  ...
  mutate(Unit = "tonnes")
write_csv(h_comm_bal_crops_2, paste0(save_path, "CommodityBalances_Crops_2017_E_All_Data_(Normalized)_7_20_22_formatted.csv"))

#read in recently downloaded commodity balances livestock data from FAO website
h_comm_bal_livestock_fish <- fread(paste0(fao_data_path, "CommodityBalances_LivestockFish_2017_E_All_Data_(Normalized)_7_20_22.csv"))
...
h_comm_bal_livestock_fish_2 <- h_comm_bal_livestock_fish_1 %>% 
  mutate(Value = Value*1000) %>% 
  mutate(Unit = "tonnes")
write_csv(h_comm_bal_livestock_fish_2, paste0(save_path, "CommodityBalances_LivestockFish_2017_E_All_Data_(Normalized)_7_20_22_formatted.csv"))
```
