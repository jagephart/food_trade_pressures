...
Filepaths
```{r}
trade_matrix_path <- "/home/shares/food-systems/social_justice_projects/food_trade_pressures/output_data/01b_calc_trade_matrix/"
...
# data check for dupes, uncomment if desired
...
### Livestock
Subset the pressure efficiency data to the livestock food group. Pivot wider to make joining with trade data easier to manage.
```{r}
...
# check for dupes, uncomment if desired to check
...
Subset the pressure efficiency data to the fiseries and mariculture food group. Pivot wider to make joining with trade data easier to manage.
```{r}
...
# check for dupes, uncomment to run data check
...
fofm_trade_matrix <- read.csv(paste0(fmfo_trade_matrix_path, "fmfo_trade_matrix_w_unknowns_allocated.csv")) %>% 
  ...
  mutate(nceas_group = "fofm",
         # we will use habitat and method as placeholders to join with the other fish data
         habitat = "fofm_feed",
         method = "fofm_feed")
...
- fish and fishmeal/fishoil trade will be combined and connect to the pressures via their country code and nceas_group category
...
crop_trade_pressures <- crop_trade_pressures %>%
  #we do not have pressure efficiency data for sugar beet so filter out SPAM_super group
  filter(SPAM_super != "sugb")
...
crop_trade_matrix_test <- crop_livestock_trade_matrix %>% 
  ...
  filter(!SPAM_super == "ofib", !SPAM_super == "othr", !SPAM_super == "teas",
         !SPAM_super == "toba", !SPAM_super == "xcof") %>% 
  # also remove sugar beet as we do not have pressures data so will not be included
  filter(SPAM_super != "sugb")
...
fish_trade_countries <- fish_trade_matrix %>%
  ...
  # filter out NEI iso3c code as it is not in other data sources
...
fish_trade_pressures_clean <- fish_trade_pressures %>% 
  ...
  mutate(food_group = "fisheries and aquaculture",
         # add in water_total columns (will be zero due to nature of food items)
         water_total = 0,
