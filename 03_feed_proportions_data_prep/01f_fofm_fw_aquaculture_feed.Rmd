...
fofm_reference <- read_csv(paste0(fw_data_path, "msleckman.45.1_fofm_consumption.csv")) %>%
  ...
  # rename current mean to true tonnes of fish 
  rename(true_tonnes_fish = Current_Mean) %>%
  # convert live_weight fofm true tonnes fed (28% reduction)
  mutate(true_tonnes_fofm = true_tonnes_fish*0.28)  
...
fofm <- left_join(fofm_reference, divided_fofm, by="fw_aquaculture") %>%
  ...
  mutate(taxa_tonnes_fofm_consumption = true_tonnes_fofm*prop) %>%
