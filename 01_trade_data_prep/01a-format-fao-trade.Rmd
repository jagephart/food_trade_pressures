...
Bulk download of trade matrix data from FAO was used and subset to years of interest. We need to gapfill missing trade data with trading partners (FAO does this for some, but not all).
...
Trade data from FAO - Gapfill imports with export data
```{r}
...
Based on the variable names, it looks like Schwarzmueller did the bulk download and then separated the data into primary crops, processed crops, and primary livestock. We will replicate this process below and subset for our years of interest.
...
Commodity and food balances are used for offal production and sugar conversions in Schwarzmueller et al 2022. We are not converting sugar to sugar cane and sugar beet as we only have sugar cane pressure efficiencies. We will assume sugar cane pressure efficiencies for sugar aggregate trade and production. We do not have offal efficiencies so those are not included in our data output.
