...
NOTE: As of right now some countries have multiple pressure efficiencies for a few items (see below list). It is unclear why this is happening presently but for now we will take the mean of any values where there is more than one and proceed with the calculations. 

- chicken meat and eggs for livestock
- xmil SPAM_super category for crops
```{r}
...
# take a look at what items have multiple pressures (xmil SPAM_super category)
...
#     dplyr::filter(n > 1L) 
# 
# pressure_crops_dupes_items <- pressure_crops_dupes %>% 
#   select(iso3c, SPAM_super, item_code, item) %>% 
#   distinct()
...
pressures_livestock_wide <- pressures_livestock %>% 
  # because of the multiple pressures for some chicken data group by and take the mean of these items,
  group_by(iso3c, nceas_group, item_code, item, pressure) %>% 
  summarise(pressure_per_tonne = mean(pressure_per_tonne, na.rm = TRUE)) %>% 
  ungroup() %>% 
  pivot_wider(names_from = pressure,
              values_from = pressure_per_tonne) %>% 
...
pressure_livestock_dupes_items <- pressure_livestock_dupes %>% 
  select(iso3c, nceas_group) %>%  #, item_code, item) %>% 
  distinct()
...
test_pressures <- pressures_crops %>% 
...
crop_trade_pressures <- left_join(x = crop_trade_countries, 
                                  ...
                                  by = c("producer_iso3c" = "iso3c",
                                         ...
                                         "SPAM_super")) %>% 
  select(- c(consumer_country_code, producer_country_code, SPAM_super, nceas_group)) %>% 
  # join with FAO SPAM and item_code data
  left_join(map_to_fao, 
            by = "item_code")
  mutate()
...
trade_pressures_na_items <- left_join(trade_pressures_na, item_codes, by = "item_code") %>% 
  select(producer_country, producer_iso3c, SPAM_super, item_code, Item) %>% 
  distinct()
...
Why are there extra rows now when we join with the pressures data? Need to create a check for many to many joins.... would mean there is multiple data for producer item codes in pressure_livestock...
