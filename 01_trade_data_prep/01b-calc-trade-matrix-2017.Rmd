---
title: "Calculating trade matrix for 2015-2019 data using Schwarzmueller 2022 code"
...
This script has been modified from the Schwarzmueller & Kastner 2022 publication to fit our needs for this data output. Largely the structure of this script remains original from that publication with annotations included to understand what the code is doing. 
...
Now choose your default options: import data, dry matter, and 2015-2019. We only have import data (gapfilled using export data in step 01a) only for 2015-2019, but we'll keep these options in here since they're used throughout the script.
```{r}
...
if(!exists("included_years")){
  # we changed the included years to be an average over these years
  included_years = "2015-2019"
}
```
...
```{r}
filename <-paste("output_data/01b_calc_trade_matrix/TradeMatrix_", prefer_import, "_", conversion_opt, "_", included_years, ".csv", sep="")
log.filename <- paste("output_data/01b_calc_trade_matrixlogfile_TradeMatrix_", prefer_import, "_", conversion_opt, "_", included_years, ".txt", sep="")
```
...
```{r}
if (!exists("directory")) { 
  directory <- "/home/shares/food-systems/social_justice_projects/food_trade_pressures"
}
setwd(directory)
```


Create a new folder in the directory (on the server) called "01b_calc_trade_matrix".
This is where we'll save methods and the final trade matrix output.
```{r}
if(!dir.exists("output_data/01b_calc_trade_matrix")) dir.create("output_data/01b_calc_trade_matrix")
```


Create and update text file. - commented out as it is not currently needed
```{r}
...
#### Read in trade data
Read in the trade data from [FAO detailed trade matrix](https://www.fao.org/faostat/en/#data/TM). This data was modified in step 01a. This data includes how much (tonnes) was traded between every country and product. For example - tonnes of refined sugar traded between Algeria and Albania.
```{r}
### Trade data
...
trade.data.file <- "output_data/01a_format_fao_trade/Trade_average_DetailedTradeMatrix_Gapfilled_formatted.csv"
...
# check NAs
length(which(is.na(trade$Value_average))) # 20,405 NAs
length(which(is.na(trade$Value_average)))/length(trade$Value_average)*100 # 1.9% of the data
...
# see if Schwarzmueller 2017 FAO data has NAs (includes import quantity, import value, export quantity, and export value)
#k_trade <- fread("Data/Trade_DetailedTradeMatrix_E_All_Data_(Normalized).csv")
#k_trade <- k_trade %>% 
#  filter(Year == 2017)
#table(k_trade$Element)
#length(which(is.na(k_trade$Value))) # they somehow have 0 NAs, suggesting Schwarzmueller did gapfilling, filtered out the data before this script or
...
#also note that there are no negative values in the trade currently!
raw.trade.data <- trade %>% 
  filter(!is.na(Value_average))
...
Information on when each country started and stopped reporting trade data. This is not important for our analysis as we have filtered the data to our years of interest already
```{r}
### Reporting Data
# specify name of file and file path you want to read in
reporting.data.file <- "raw_data/Codes/Reporting_Dates.xls"
...
#### Read in item information and conversions data
Read in the primary product item data. This includes information on processed products and their FAO codes and the primary items they are associated with and their FAO codes. 

For example - orange juice concentrated (FAO code 492) is next to oranges (FAO code 490). There are 550 lines, so 550 items. Livestock have a "0" for their associated primary product FAO code and an "NA" for their associated primary product name. Therefore they are already considered to be in their primary product format.
```{r}
### Primary item data
# specify name of file and file path you want to read in
item.map.file <- "raw_data/Codes/primary_item_map_feed.csv"
...
##Conversion factors
# specify name of file and file path you want to read in
content.factors.file <- "raw_data/Codes/content_factors_per_100g.xlsx"
...
These data include: for each country, the area harvested, yield, and production for each crop for all years.
```{r}
### Production data - crops

# specify name of file and file path you want to read in
production.crops.file <- "output_data/01a_format_fao_trade/Production_Crops_2015_2019_FAOSTAT_6_29_2023_formatted.csv"
...
These data include: for each country, various measurements of production for livestock (e.g., egg production, camel meat yield)
```{r}
### Production data - animal products

# specify name of file and file path you want to read in
production.animals.file <- "output_data/01a_format_fao_trade/Production_LivestockPrimary_2015_2019_FAOSTAT_6_29_2023_formatted.csv"
...
Read in offals data.
Offals = the entrails and internal organs of an animal used as food. Data includes production of edible offals in tons for each country. This section is included in the original Schwarzmueller script and will remain for now however, offals are not included in our final data output as we do not have any pressures data to assign it to.
```{r}
### Production data - offals

# specify name of file and file path you want to read in
production.offals.file <- "output_data/01a_format_fao_trade/CommodityBalances_2015_2019_(Normalized)_06_29_2023_formatted.csv"
...
# for Schwarzmueller's data using element code 5510, but that isn't working for our newly
...
# Remove first two columns - these no longer exist but may have in Schwarzmueller data
...
Filter the production data to only include countries (area_codes < 300). Higher area codes are groups of countries (e.g., Europe).

Also filter the data to only include data measured as production in tonnes (element code 5510 and 5511 for offals).
```{r}
table(production.all$Element_Code)

# have to include production code 5511 for offals too (not in Schwarzmueller script)
production.all <- filter(production.all, 
                         Area_Code<300, 
                         Element_Code==5510 | Element_Code==5511)
```

Delete the individual production dataframes from the environment if desired - they are all in production.all
```{r}
...
#### Sugar conversions data
Read in the sugar conversion data. This is done in the original Schwarzmueller script so it has been kept in however, our pressure data does not have efficiencies for sugar beet so we will not divide the sugar aggregate.

These data include: for each country, all elements (e.g., production, import quantity, export quantity, domestic supply quantity, waste, etc.) for every product (apples, barley, etc.) in tonnes. This is not subset to just sugar yet, we  do this later.
```{r}
### Data files for sugar conversion
# specify name of file and file path you want to read in
sugar.processing.file <- "output_data/01a_format_fao_trade/CommodityBalances_2015_2019_(Normalized)_06_29_2023_formatted.csv"
...
### Conduct import and export dataframes
Create import and export dataframes. We have already done this in step 01a. So not necessary to do for our purposes but kept in original from Schwarzmueller script.
```{r}
...
data.import <- select(raw.trade.data, Reporter_Country_Code, Partner_Country_Code, Element_Code, Item_Code, Year, Value_average) %>%
...
#### Remove dates function
Create function to eliminate years without reports. Schwarzmueller et al 2022 uses data back to the 1980s, since we'll only be using an average from 2015-2019 data, we don't need to do this.
```{r}
...
#### Remove zeros and where imports == exports country
...
# this function does nothing to the average trade data
length(which(data.import$Value_average == 0)) #304433
length(which(raw.trade.data$Value_average == 0)) #304433

# remove reporting.data dataframe from environment
# rm(reporting.date)
```

Eliminate rows where the exporting and importing countries are the same country (set them to 0 then eliminate). "Self" traded data will be generated from the matrix operation. Also, eliminates rows where Value = 0, which includes rows with no data reported (we set those rows to 0 in `eliminate.dates` function previously).
```{r}
# set Value_average column to 0 where importing and exporting countries (cols 1 and 2) are the same
data.import <- data.import %>% 
  mutate(Value_average = if_else(condition = Reporter_Country_Code == Partner_Country_Code,
                                 true = 0,
                                 false = Value_average))
#data.import[which(data.import[,Reporter_Country_Code]==data.import[,Partner_Country_Code]), Value_average] <- 0
...
data.import <- filter(data.import, Value_average !=0)
...
Only taking relevant parts from previous code chunk for our needs. This is not in the original script.
```{r}
# update text file
...
trade.data <- data.import %>% 
  distinct(Consumer_Country_Code, Producer_Country_Code,  Year, Item_Code, .keep_all=T) # () variable removed because of average
```
...
Remove `data.import` and `data.export` from the environment if desired.
```{r}
...
Function to create conversion factors for each product - divide processed products dry matter content by their primary products dry matter content. For example, wheat would be 0.86/0.86 = 1, wheat flour would be 0.88/0.86 = 1.02.
```{r}
...
#### Convert trade data to primary products
Join the trade data with the conversion factor data (get this from `primary.conversion` function)
```{r}
#### map to primary equivalents ####
trade.data <- left_join(trade.data, 
                        primary.conversion(conversion_map = item.map,
                                           conversion_option = conversion_opt, 
                                           display = T), 
                        by=c("Item_Code"="FAO_code"))
```


Create a new column in the trade data called `primary.Value`, which is the value of traded product (tonnes) times the conversion factor. Offals have a conversion factor of 0, so all offals get the Value = 0.

"Rice, paddy (rice milled equivalent)" is referred to as "Rice <96> total  (Rice milled equivalent)" in this dataframe.
```{r}
trade.data <- mutate(trade.data, primary.Value= Value_average * Conversion_factor)
```


Group the `trade.data` by the consumer country code, producer country code, year, and primary item. Then create a new column called `Value.Sum`, which is the sum of all the `primary.Value` for that grouping (sum of all processed products for 1 primary item).

Ungroup the data and remove all rows where the `primary_item` = 0  or the `Value.Sum` = NaN. You end up with a dataframe (`primary.data`), which shows how many tonnes of each primary product is traded between each country on average from 2015-2019 (processed products are not included as they have been converted to primary product tonnes).

Multiple processed products from the same primary product (e.g., apple juice, cooked apples) are summed to just get the total tonnes of apples traded between countries.

Offals are removed at this stage since they have a conversion value of zero and will not be included in our output anyways.
```{r}
...
# remove the trade.data if desired
# rm(trade.data)
```

### Sugar Conversion Setup
This step is original to the Schwarzmueller et al 2022 script and parts are not being included in our data structure as we do not have sugar beet pressures available. These code chunks are commented out but remain here to maintain the original script. 
```{r}
...
# sugar.production <- left_join(filter(production.all,
#                                      Item_Code %in% sugar.crop.codes),
#                               primary.conversion(conversion_map = item.map,
#                                                  conversion_option = conversion_opt,
#                                                  display = F),
#                               by = c("Item_Code" = "FAO_code"))
...
# sugar.production$Value.new <- sugar.production$Value_average * sugar.production$Conversion_factor
```
...
#   mutate(share = Value_average/sum(Value_average, na.rm=T)) %>%
#   ungroup() %>%
...
#   summarise(Value_average = sum(Value.new, na.rm=T)) %>%
#   ungroup() %>%
#   # remove countries that have 0 for sugar production
#   filter(Value_average > 0) %>%
...
### Matrix operation
The above steps were conducted to set up the trade matrix operation. This section executes the matrix function.

#### Matrix function setup
For the `matrix.operation` function to work properly the year values need to be single years or possible to be numerical values. Currently our data year column is labeled 2015-2019 to denote that it is an average over that time span. The below code chunk will mutate the year column to just be 2017 so the rest of the operations can work as expected becasue it requires a number.
```{r}
primary.data <- primary.data %>% 
  # Year changed to 2017 for the sake of the calculations set up by Schwarzmueller
  mutate(Year = 2017)
production.all <- production.all %>% 
  # Year changed to 2017 for the sake of the calculations set up by Schwarzmueller
  mutate(Year = 2017)
...
#### Matrix Function
Function containing matrix calculations to calculate consumption data for each country according to the country of origin. Country of origin being where the product was actually produced, not just the last country it passed through for processing.
```{r}
matrix.operation <- function(item_Code, year){
 ...
 # subset the production.all data to the specified item code, year, and element code 5510 (keep all variables)
  ...
  production_data <- full_join(prod, select(production, Area_Code, Value_average), by= "Area_Code") # Value_average instead of Value to reflect new average data
...
for (i in (1:nrow(q))){
  ...
  transformed.data <- rbindlist(list(transformed.data, 
                                     matrix.operation(item_Code, year)))
}

# remove rows where the consumer_country_code = 0 
...
#### Identify missing products after matrix operation
...
missing.prod.data <- production.all %>% 
  ...
  filter(Element_Code==5510, Year %in% c(2017), Item_Code !="NA", Value_average!="NA") %>% # c(2017) was `included_years` but was changed to reflect averaging
...
#1,162 rows of data in missing.prod.data
...
add.data <- missing.prod.data %>%
  # create new columns for producing and exporting countries that are the country's area code
  ...
  select(Consumer_Country_Code, Producer_Country_Code, Value = Value_average, Item_Code, Year)

#add missing production data to consumption trade data
transformed.data <- rbind(transformed.data, add.data)
```

### Sugar Cane vs. Sugar Beet Processing
Sugar cane vs. sugar beet processing conversions. For our purposes we are not including this step in our data product as we do not have sugar beet pressures. All sugar aggregate product will receive sugar can pressures. The code for this processing is commented out but will remain in this section as it is part of the original script from Schwarzmueller et al. 2022.
...
# sugar.processing <- filter(sugar.processing, Item_Code %in% c(2536,2537), Element_Code==5131, Area_Code<300, Value_average>0)
# 
...
# sugar.processing$Value.new <- sugar.processing$Value_average * sugar.processing$Conversion_factor
# 
...
More sugar cane vs. sugar beet processing converstions. For our purposes we are not including this step in our data product as we do not have sugar beet pressures. All sugar aggregate product will receive sugar can pressures.
...
#   # Make year 2017 so it matches the rest of the data
#   mutate(Year = 2017) %>% 
#   group_by(Area_Code, Year) %>%
...
#   rename(national_production = Value_average)
# 
...
### Remove Negative Trade and Save
Due to discrepancies in production and trade data, along with variability in primary commodity conversion factors, there are cases where exports exceed production and imports. This results in negative consumption values. Across all producing to consuming country flows by food item, there are are 2.78% percent of observations with a value less than -1 t, representing less than 0.3% of volume, and 0.66% of observations with less than -100 t. Since negative consumption is not possible, we replace all negative values with zero. This step was not included in the orginal script from Schwarzmueller et al 2022.
```{r}
transformed.data <- transformed.data %>% 
  mutate(Value = if_else(condition = Value < 0,
                         true = 0,
                         false = Value))

```

Save the trade matrix data results that is an average of 2015-2019 data and removes the negative trade values from the matrix.
```{r}
#### write file ####
fwrite(transformed.data, "output_data/01b_calc_trade_matrix/trade_matrix_by_country_of_origin_2015_2019_import.csv")
```

### Data Checks Not Originally in Schwarzmueller et al 2022 
Code below is checking Schwarzmueller calculations with FAO production data.
This is all code I (Haley) wrote, none of this is from the Schwarzmueller paper.
...
country_production <- transformed.data %>% 
  ...
  mutate(Schwarzmueller_production = sum(Value)) %>%
  ungroup()
...
country_production_cut <- country_production %>% 
  select(Producer_Country_Code, Item_Code, Schwarzmueller_production) %>% 
  unique(.)
...
# same countries in Schwarzmueller trade matrix and FAO production data
...
faostat_cut <- production.FAO.2 %>% 
  ...
  select(Area_Code, Area, Item_Code, Item, Year, Value_average) %>% 
  rename(FAO_production = Value_average)

# join dataframes by area code and item code
fao_and_Schwarzmueller_production <- full_join(faostat_cut, country_production_cut, by = c("Area_Code" = "Producer_Country_Code", "Item_Code" = "Item_Code"))

# rename columns
#fao_and_Schwarzmueller_production <- fao_and_Schwarzmueller_production %>% 
#  rename("area_code" = "Area Code (FAO)") %>% 
#  rename("item_code" = "Item Code (FAO)")

# remove rows where both values for production are NA (801 rows)
fao_and_Schwarzmueller_production_1 <- fao_and_Schwarzmueller_production %>% 
  filter(!(is.na(FAO_production) & is.na(Schwarzmueller_production)))

# remove rows where one value is NA and the other value is 0 (291 rows)
fao_and_Schwarzmueller_production_2 <- fao_and_Schwarzmueller_production_1 %>% 
  filter(!(is.na(FAO_production) & Schwarzmueller_production == 0)) %>% 
  filter(!(is.na(Schwarzmueller_production) & FAO_production == 0))
...
# FAO trade matrix, not just China. Therefore, China is not included in the Schwarzmueller
# trade matrix, but is included in the FAO production data we are comparing it to here.

# remove China (351)
fao_and_Schwarzmueller_production_3 <- fao_and_Schwarzmueller_production_2 %>% 
  filter(!Area_Code == 351)

# plot - compare FAO and Schwarzmueller production with scatterplot - looks great!
ggplot(fao_and_Schwarzmueller_production_3, aes(x = FAO_production, y = Schwarzmueller_production)) +
  geom_point() +
  xlab("FAO production value") + ylab("Schwarzmueller production value") +
  theme_bw()

# look for missing values
length(which(is.na(fao_and_Schwarzmueller_production_3$FAO_production))) #0
length(which(is.na(fao_and_Schwarzmueller_production_3$Schwarzmueller_production))) #2186

# look at NAs in trade matrix - 2,186 rows
nas <- fao_and_Schwarzmueller_production_3 %>% 
  filter(is.na(Schwarzmueller_production))
table(nas$Item)
# most of what are included in the FAO data but not in Schwarzmueller are from the "Crops Processed > (List)"
# https://www.fao.org/faostat/en/#data/QCL , which Schwarzmueller did not include these items when downloading
# FAO data

# check to see if Schwarzmueller included these processed crops when he downloaded FAO production data
test <- fread("raw_data/Schwarzmueller_2022/Production_Crops_E_All_Data_(Normalized).csv")
table(test$Item)
# NO - they did not include anything listed under "Crops Processed > (List)"
# https://www.fao.org/faostat/en/#data/QCL

# There are some additional products included in FAO production but not Schwarzmueller trade matrix:
...
# In the Schwarzmueller trade matrix:
...
#    included in the FAO trade matrix, so they're not included in the Schwarzmueller trade matrix
# 3. All offals were converted to "offals edible" in the Schwarzmueller trade matrix when converting
#    to primary products, but the conversion factor was 0, so Value was set to 0, and all rows
#    with Value=0 were removed, so the Schwarzmueller trade matrix does not have any offals.
