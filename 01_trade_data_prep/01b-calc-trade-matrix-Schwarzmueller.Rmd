...
Set working directory (if not previously specified in _Execute.R). We'll set our working directory as the server as that is where the data are located.
```{r}
if (!exists("directory")) { 
...
Read in the reporting data using `read_xls` since it is an Excel file rather than csv. Information on when each country started and stopped reporting trade data. This is not important for our analysis as we have filtered the data to our years of interest already, this is just from the cited paper.
```{r}
### Reporting Data
...
Read in conversions factors data. This dataset contains information on the conversion from primary to processed products in dry weight,protein, calories, and some vitamins. This dataset does not indicate which primary product each processed product comes from - that is the `item.map`.
```{r}
##Conversion factors
# specify name of file and file path you want to read in
...
Read in offals data.
Offals = the entrails and internal organs of an animal used as food. Data includes production of edible offals in tons for each country. This section is included in the original Schwarzmueller script however, offals are not included in our final data output as we do not have any pressures data to assign it to.
```{r}
...
Filter the production data to only include countries (area_codes < 300). Higher area codes are groups of countries (e.g., Europe). Also filter the data to only include data measured as production in tonnes (element code 5510 and 5511 for offals).
```{r}
...
#### Sugar conversions data - used to track sugar cane vs sugar beet
Read in the sugar conversion data. This is done in the original Schwarzmueller script however it should be noted that our pressure data does not have efficiencies for sugar beet so its trade will eventually be removed. This represents a < 1% of the total volume of goods traded.
...
trade.data <- data.import %>% 
  # () variable removed because of average
  distinct(Consumer_Country_Code, Producer_Country_Code,  Year, Item_Code, .keep_all=T) 
```
...
Create a new column in the trade data called `primary.Value`, which is the value of traded product (tonnes) times the conversion factor. Offals have a conversion factor of 0, so all offals get the Value = 0. Note, "Rice, paddy (rice milled equivalent)" is referred to as "Rice <96> total  (Rice milled equivalent)" in this dataframe.
```{r}
...
As noted earlier, sugar beet trade/production will eventually be removed from our data as there is not pressure efficiency data available. These steps are kept in to complete the original processing done by Schwarzmueller et al. (2022). Without this step the sugar data will be lost in the matrix operation because the primary item code for sugar aggregate is not the same as the `Item_Code`.
...
Calculate the tonnes of sugar in primary product (beet and cane) equivalents that each country produces.
```{r}
...
For the `matrix.operation` function to work properly the year values need to be single years or possible to be numerical values. Currently our data `year` column is labeled 2015-2019 to denote that it is an average over that time span. The below code chunk will mutate the year column to just be 2017 so the rest of the operations can work as expected because it requires a number in the year column.
```{r}
...
Create keys for primary.data and production.all. "setkey sorts a data.table and marks it as sorted with an attribute sorted. The sorted columns are the key. The key can be any number of columns." The columns are always sorted in ascending order. The table is changed by reference.
```{r}
...
Sugar cane vs. sugar beet processing conversions. Create a df with proportion of beets vs cane a country produces and proportion of beets vs cane a country processes into sugar. Use the trade and production data to adjust the `transformed.data` for sugar cane and beet trade accordingly. As mentioned before, sugar beet will eventually be removed from our data as we do not have environmental pressures data.
...
Add new column (control) that indicates whether sugar is produced in a country but not processed in that country. If sugar is processed but not produced in a country, set control to 0 (rather than  NA)
```{r}
...
```{r}
# adjust domestic component (diagonal in the trade matrix operation)
...
Due to discrepancies in production and trade data, along with variability in primary commodity conversion factors, there are cases where exports exceed production and imports. This results in negative consumption values. Across all producing to consuming country flows by food item, there are are 2.78% percent of observations with a value less than -1 t, representing 0.3% of volume, and 0.66% of observations with less than -100 t. Since negative consumption is not possible, we replace all negative values with zero. This step was not included in the orginal script from Schwarzmueller et al 2022.

```{r}
# uncomment to view negative trade
# test_negative <- transformed.data %>% 
#   filter(Value<0)
