...
This section will prepare the FOFM production data as the trade data does not cover this well.
...
commodity_list <- read.csv(file = paste0(production_path, "CL_FI_COMMODITY_ISSCFC.csv")) %>% 
  # keep code cololumn and english name
  select(Code, Name_En) %>% 
  filter(str_detect(string = Name_En, pattern = "meal") | str_detect(Name_En, pattern = "oil"),
         # don't include boiled products
         !str_detect(Name_En, pattern = "boil"),
         # do not include not minced products that are preserved in oil
         !str_detect(Name_En, pattern = "not minced")) %>% 
...
Average production over years
```{r}
production_average <- production_clean %>% 
  group_by(ISO3_Code, country_name, product, product_code) %>% 
  summarise(tonnes_product_weight = mean(VALUE))
```
