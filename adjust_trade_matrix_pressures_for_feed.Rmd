...
prop_feed <- read_csv("scripts/feed_in_trade_matrix/outputs/all_crops_perc_feed.csv")
...
Read in tonnes crop consumed per animal group per country data :
https://github.com/OHI-Science/food_systems/blob/master/feed/data/system_country_mapspam_tonnes_consumption.csv
- go to url, click "View Raw", copy url and paste it below. Need to re-do this every time because the url expires.
```{r}
animal_consumption <- read_csv("https://raw.githubusercontent.com/OHI-Science/food_systems/master/feed/data/system_country_mapspam_tonnes_consumption.csv?token=GHSAT0AAAAAABXM6CBNSZ747HNMOHE667FUY337HKA")
```
...
Mariculture (we have pressures data for bivalves but no feed data, maybe they don't need feed because they filter?):
...
animal_consumption_3 <- animal_consumption_2 %>% 
  mutate(animal_agg = case_when(str_detect(animal_system, regex("milk", ignore_case = TRUE)) ~ "milk",
                                ...
                                animal_system == "crustaceans_aquaculture_meat" ~ "crustaceans_aquaculture",
                                animal_system == "marine-fish-general_aquaculture_meat" ~ "marine_fish_general_aquaculture",
                                animal_system == "salmon_aquaculture_meat" ~ "salmonids_aquaculture",
                                animal_system == "shrimp_aquaculture_meat" ~ "shrimps_prawns_aquaculture",
                                animal_system == "tuna_aquaculture_meat" ~ "tuna_aquaculture",
                                TRUE ~ animal_system))
...
Read in inland aquaculture feed consumption 
https://github.com/jagephart/food_trade_ej/blob/main/scripts/fw_aquaculture_pressures/output/fw_aqua_crop_feed.csv
```{r}
fw_aqua_consumption <- read_csv("https://raw.githubusercontent.com/jagephart/food_trade_ej/main/scripts/fw_aquaculture_pressures/output/fw_aqua_crop_feed.csv?token=GHSAT0AAAAAABXM6CBM2XEZRJQXH3AR5HC6Y337HVQ")
```
...
fw_aqua_consumption_relabel <- fw_aqua_consumption %>% 
  mutate(nceas_group = case_when(taxa == "catfish" ~ "Miscellaneous freshwater fishes",
                                 taxa == "hypoph_carp" ~ "Carps, barbels and other cyprinids",
                                 taxa == "milkfish" ~ "Miscellaneous diadromous fishes",
                                 taxa == "misc_diad" ~ "Miscellaneous diadromous fishes",
                                 taxa == "oth_carp" ~ "Carps, barbels and other cyprinids",
...
# 2396 NAs in pressures df means we have the proportion of crop that animal consumes in that country, but we don't have any pressures data so it's not included in the crops trade matrix 
# look at NAs
...
((1621789997 - 1613395685)/1621789997)*100 # 0.52% = less than one percent, ignore this discrepancy
...
fish <- read_csv("/home/shares/food-systems/social_justice_projects/food_trade_ej/data/capture_and_mariculture_fish_products_trade_matrix_w_pressures.csv")
aqua <- read_csv("/home/shares/food-systems/social_justice_projects/food_trade_ej/data/inland_aquaculture_products_trade_matrix_w_pressures.csv")

# format trade matrices so they can be combined into one df
animals_1 <- animals %>% 
  select(Consumer_ISO3, Producer_ISO3, prop_traded, item_combo) %>% 
  rename(product = item_combo)
animals_1 <- animals_1[, c(2, 1, 4, 3)]
fish_1 <- fish %>% 
  select(source_country_iso3c, importer_iso3c, nceas_group, prop_traded) %>% 
...
all_animals_2 <- all_animals_1 %>% 
  rename(animal_producing_country = Producer_ISO3,
         ...
         prop_animal_traded = prop_traded)
...
animal_feed_pressures_tm <- full_join(feed_pressures_4, all_animals_2, by = c("animal_producing_country", "product"))
...
# 72,049 NAs for pressures - have animal trade data for an animal system, but no feed trade data 

no_animal_data <- animal_feed_pressures_tm %>% 
  filter(is.na(prop_animal_traded))
# 100,608 NAs for prop_animal_traded - have feed data, but no animal trade data
...
# how many rows are we discarding? Only 0.66%.
...
Save df
```{r}
write_csv(animal_feed_pressures_tm_3, "/home/shares/food-systems/social_justice_projects/food_trade_ej/data/animal_feed_trade_matrix_w_pressures.csv")
```
...
# save csv
write_csv(non_feed_crops_tm, "/home/shares/food-systems/social_justice_projects/food_trade_ej/data/non_feed_crops_trade_matrix_w_pressures.csv")
```
...
```{r}
non_feed_crops_trade <- read_csv("/home/shares/food-systems/social_justice_projects/food_trade_ej/data/non_feed_crops_trade_matrix_w_pressures.csv")
animal_feed_trade <- read_csv("/home/shares/food-systems/social_justice_projects/food_trade_ej/data/animal_feed_trade_matrix_w_pressures.csv")
capture_mariculture_trade <- read_csv("/home/shares/food-systems/social_justice_projects/food_trade_ej/data/capture_and_mariculture_fish_products_trade_matrix_w_pressures.csv")
fw_aquaculture_trade <- read_csv("/home/shares/food-systems/social_justice_projects/food_trade_ej/data/inland_aquaculture_products_trade_matrix_w_pressures.csv")
livestock_trade <- read_csv("/home/shares/food-systems/social_justice_projects/food_trade_ej/data/animal_products_trade_matrix_w_pressures.csv")

# format data so we can rbind it

# order = producer, consumer, item, tonnes traded, cumu, ghg, water, nutrient, disturbance
non_feed_crops_trade <- non_feed_crops_trade[,c(2, 1, 4, 3, 5:9)]
...
animal_feed_trade_1 <- animal_feed_trade_1[,c(1:2, 9, 3, 4, 6, 5, 7:8)]

summary(capture_mariculture_trade) #1 NA
capture_mariculture_trade_1 <- capture_mariculture_trade %>% 
  select(-c(Producer_country, sum_caught, prop_traded)) %>% 
  rename(Producer_ISO3 = source_country_iso3c,
         Consumer_ISO3 = importer_iso3c,
         Item = nceas_group,
         tonnes_traded = trade_weight) %>% 
  filter(!is.na(trade_cumu_pressure))

summary(fw_aquaculture_trade) # 3,129 NAs
...
livestock_trade_1 <- livestock_trade %>% 
  select(Producer_ISO3, Consumer_ISO3, item_combo, tonnes_traded, trade_cumu_pressure,
         trade_ghg, trade_water, trade_nutrient, trade_disturbance) %>% 
  rename(Item = item_combo)
...
# save csv
write_csv(df_4, "/home/shares/food-systems/social_justice_projects/food_trade_ej/data/trade_matrix_w_pressures_accounting_for_feed.csv")
```
