...
seafood_known <- seafood_matrix %>%
  filter(source_country_iso3c != "unknown",       # Remove rows with unknown source seafood
         nceas_group != "unknown") %>%            # Remove rows with unknown nceas_group but known source, habitat, and method,
  group_by(consumer_iso3c, source_country_iso3c, 
           habitat, method) %>%                   # Calculate each consumer country's proportion of total consumption by source country, habitat, and method
  mutate(consumer_prop_by_source_method =
...
# Note - there are no instances in seafood_matrix where the habitat is known and the method is unknown, uncomment code below to check
# habitat_known_method_unknown <- seafood_matrix %>% 
#   filter(source_country_iso3c != "unknown", 
#          habitat != "unknown",
#          method == "unknown")

# Separate volumes of known source and known habitat but unknown nceas_group
unknown_seafood_w_source_method <-  seafood_matrix %>% 
  filter(source_country_iso3c != "unknown", 
         habitat != "unknown",
         method != "unknown",
         nceas_group == "unknown") %>% 
  select(consumer_iso3c, source_country_iso3c, habitat, method,
         "unknown_w_source_method_consumption_live_weight_t" = "consumption_live_weight_t") # change name of consumption_live_weight_t column)
...
seafood_known  <- seafood_known %>%
  # join with unknown_seafood_w_source_method
  left_join(unknown_seafood_w_source_method,
            by = c("consumer_iso3c", "source_country_iso3c", "habitat", "method"),
            fill = NA) %>% 
  mutate(unknown_w_source_method_consumption_live_weight_t =                          
           replace_na(unknown_w_source_method_consumption_live_weight_t, 0)) %>%         # make any NAs in unknown_w_source_method_consumption_live_weight_t zero
  mutate(consumption_live_weight_t =                                                     # add to consumption_live_weight_t
           consumption_live_weight_t + 
           unknown_w_source_method_consumption_live_weight_t*
           consumer_prop_by_source_method) %>%                                           # by multiplying unknown_w_habitat_consumption_live_weight_t by the consumer_prop_by_source_method
  
  # join with unknown_seafood_w_habitat
  left_join(unknown_seafood_w_habitat,                                                   
  ...
  # now join with unknown_seafood_wout_habitat
  left_join(unknown_seafood_wout_habitat,                                                
            by = "consumer_iso3c",
            fill = NA) %>% 
...
check_consumption <- seafood_matrix %>%                                             
  ...
  mutate(difference_consumption_live_weight_t = 
           original_consumption_live_weight_t -     
           new_consumption_live_weight_t)
View(check_consumption)

nrow(seafood_known %>%
       filter(source_country_iso3c == "unknown" | habitat == "unknown" | method == "unknown"))
```

169 countries total consumption is different by more than 10^-6 with the `new_consumption_live_weight_t`. Currently unsure what needs to be done for these countries and will bring it to the group.

There should be no rows in `seafood_known` where the `consumer_iso3c`, `habitat`, or `method` are equal to unknown. 
