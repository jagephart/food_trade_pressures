...
livestock_fish_trade_pressures_for_final <- livestock_fish_trade_raw %>% 
  # remove fofm_feed item as it has been accounted for elsewhere
  filter(item != "fofm_feed") %>% 
  select(producer_iso3c, consumer_iso3c, item, food_group,
         tonnes_traded = traded_tonnes, 
         ghg_traded = ghg_total, 
...
# check data - exclude FOFM for now because we are determining how to proceed with feed allocation
check_trade <- trade_w_pressures_no_feed 

sum(trade_matrix_w_pressures_accounting_for_feed$tonnes_traded)
sum(check_trade$traded_tonnes) - sum(trade_matrix_w_pressures_accounting_for_feed$tonnes_traded)

```

```{r}
fofm_check <- trade_matrix_w_pressures_accounting_for_feed %>% 
  filter(str_detect(string = item,
                    pattern = "fofm_feed"))

raw_fofm <- trade_w_pressures_no_feed %>% 
  filter(str_detect(string = item,
                    pattern = "fofm_feed"))

sum(fofm_check$tonnes_traded)
sum(raw_fofm$traded_tonnes) - sum(fofm_check$tonnes_traded)
# there is more fofm in the original trade data than our new data

# fofm_feed check shows that there is no difference between raw data vs new data
# fofm check shows that there is 269811.3 tonnes missing in the new data
# livestock check shows 150598.9 tonnes missing in the new data

new_check <- livestock_fish_trade_pressures_for_final %>% 
  filter(food_group == "livestock") %>% 
  group_by(item) %>% 
  summarise(tonnes_traded_new = sum(tonnes_traded))

old_check <- trade_w_pressures_no_feed %>% 
  filter(food_group == "livestock") %>% 
  group_by(item) %>% 
  summarise(tonnes_traded_old = sum(traded_tonnes))

check_diffs <- left_join(new_check, old_check) %>% 
  mutate(diff = tonnes_traded_old - tonnes_traded_new)

#livestock_fish_trade_pressures_for_final
...
old_check <- trade_w_pressures_no_feed %>% 
  filter(food_group == "livestock") %>% 
  filter(is.na(item))
sum(old_check$traded_tonnes) - sum(new_check$tonnes_traded)

```

## difference is from livestock. 
