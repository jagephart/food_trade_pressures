...
save_path <- "/home/shares/food-systems/social_justice_projects/food_trade_pressures/output_data/02ai_fw_aquaculture_pressures/"
```
...
raw_production <- read_csv(paste0(fao_data_path, "Aquaculture_Quantity.csv")) %>%
  ...
  # filter for 2017 (may need to do 2015-2019)
  filter(PERIOD %in% c(2017)) %>% #c(2015, 2016, 2017, 2018, 2019)
...
tonnes_lw <- production_update %>%
  ...
  # calculate the total tonnes_lw for that group
...
# using the stressor_group column, join the tonnes_lw data with pressure_per_tonne data
pressures_lw <- left_join(tonnes_lw, pressures_converted, by="stressor_group") %>%
  # calculate the stressor value
  mutate(stressor_value = tonnes_lw*pressure_per_tonne) 

# look at data for each stressor group
pressures_lw %>%
  group_by(stressor_group, pressure) %>%
...
# Save data
#write_csv(pressures_lw, paste0(save_path, "fw_aquaculture_pressures.csv"))
```

#### Should This be where we split up this document?
...
We will use the values from Halley et al. and distribute distribute consumption of fofm based on production.
...
Need to make a few adjusments to divide categories.
...
# this also generally matches production
raw_production <- read_csv(paste0(fao_data_path, "Aquaculture_Quantity.csv")) %>%
...
salmon_test <- production_update %>%
  filter(ISSCAAP_Group_En == "Salmons, trouts, smelts") %>% 
  group_by(taxa_name) %>%
  summarize(tonnes = sum(tonnes_lw))
```
...
# take a look at each group to make sure it is the same as the fofm df
fofm_tonnes %>% 
  group_by(taxa) %>%
