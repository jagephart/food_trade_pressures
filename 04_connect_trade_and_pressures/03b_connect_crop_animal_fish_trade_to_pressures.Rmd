...
fw_aquaculture_trade_pressures_path <- "/home/shares/food-systems/social_justice_projects/food_trade_pressures/output_data/03a_connect_fw_aquaculture_pressures/"
...
save_path <- "/home/shares/food-systems/social_justice_projects/food_trade_pressures/output_data/03b_connect_crop_livestock_oth_fish_pressures/"
```
...
# fill in Taiwan Region and subregion
pressures_crops_regions$region[pressures_crops_regions$iso3c == "TWN"] <- "Eastern Asia"
pressures_crops_regions$subregion[pressures_crops_regions$iso3c == "TWN"] <- "Eastern Asia"
...
pressures_crops_regions$subregion[pressures_crops_regions$iso3c == "XKO"] <- "Southern Europe"
...
pressures_crops_subregions <- pressures_crops_regions %>% 
  group_by(subregion, SPAM_super, pressure) %>% 
  # count number of pressures used in calculation
  summarise(n = dplyr::n(),
            # calculate means of groupings 
            subregion_pressure_per_tonne = mean(pressure_per_tonne, na.rm = TRUE)) %>% 
  ungroup() %>% 
  # filter only for regional averages that are made up of more than 1 data point
  filter(n > 1)

pressures_crops_subregions_wide <- pressures_crops_subregions %>% 
  # remove count column
  select(-n) %>% 
  # pivot wide to merge with other data easily
  pivot_wider(names_from = pressure,
              values_from = subregion_pressure_per_tonne) %>% 
  rename(subregion_ghg = ghg,
         subregion_disturbance = disturbance,
         subregion_nutrient = nutrient,
         subregion_water = water)
...
# na check
region_na <- pressures_livestock_regions %>% 
  filter(is.na(region))

# fill in Taiwan Region
pressures_livestock_regions$region[pressures_livestock_regions$iso3c == "TWN"] <- "Eastern Asia"
pressures_livestock_regions$subregion[pressures_livestock_regions$iso3c == "TWN"] <- "Eastern Asia"
...
# calculate subregional averages
pressures_livestock_subregions <- pressures_livestock_regions %>% 
  group_by(subregion, item, item_code, pressure) %>% 
  # count number of pressures used in calculation
  summarise(n = dplyr::n(),
            # calculate means of groupings 
            subregion_pressure_per_tonne = mean(pressure_per_tonne, na.rm = TRUE)) %>% 
...
pressures_livestock_subregions_wide <- pressures_livestock_subregions %>% 
  # remove count column
  select(-n) %>% 
  # pivot wide to merge with other data easily
  pivot_wider(names_from = pressure,
...
pressures_fish_regions$subregion[pressures_fish_regions$iso3c == "TWN"] <- "Eastern Asia"

# the iso3c code is presumed to be for Kosovo and will therefor have a designation of Southern Europe like neighboring countries
pressures_fish_regions$region[pressures_fish_regions$iso3c == "XKO"] <- "Southern Europe"
pressures_fish_regions$subregion[pressures_fish_regions$iso3c == "XKO"] <- "Southern Europe"
...
# calculate subregional averages
pressures_fish_subregions <- pressures_fish_regions %>% 
  group_by(subregion, nceas_group, pressure) %>% 
  # count number of pressures used in calculation
  summarise(n = dplyr::n(),
            # calculate means of groupings 
            subregion_pressure_per_tonne = mean(pressure_per_tonne, na.rm = TRUE)) %>% 
...
pressures_fish_subregions_wide <- pressures_fish_subregions %>% 
  # remove count column
  select(-n) %>% 
  # pivot wide to merge with other data easily
  pivot_wider(names_from = pressure,
...
crop_trade_pressures_gapfilled <- left_join(x = crop_trade_pressures_missing, 
  ...
  select(-c( region_disturbance, region_ghg, region_nutrient, region_water)) %>% 
  # calculate the total pressure traded by multiplying traded_tonnes by the pressure efficiency
  mutate(disturbance_total = disturbance*traded_tonnes,
         water_total = water*traded_tonnes,
         ghg_total = ghg*traded_tonnes,
...
livestock_trade_pressures <- livestock_trade_pressures %>% 
  # Filter for rows with no missing pressures
  filter(!is.na(ghg) & !is.na(nutrient) & !is.na(disturbance) & !is.na(water)) 

nrow(livestock_trade_pressures) + nrow(livestock_trade_pressures_gapfilled)

livestock_trade_pressures <- rbind(livestock_trade_pressures, livestock_trade_pressures_gapfilled)
sum(livestock_trade_pressures$traded_tonnes)

# no loses, livestock is done!
```
...
# less than 1% of total tonnes traded.
```
...
nrow(fish_trade_pressures) + nrow(fish_trade_pressures_gapfilled)
...
### Livestock
livestock_trade_pressures_clean <- livestock_trade_pressures %>%
  # remove item column 
  select(-item) %>% 
...
### Freshwater Aquaculture - made in 03a
...
```{r}
write.csv(x = trade_pressures_final,
