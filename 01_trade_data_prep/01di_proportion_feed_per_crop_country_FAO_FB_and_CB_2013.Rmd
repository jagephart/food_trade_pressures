...
```
Basically just need to do the above instead of everything happening below here. Much simpler and concise but need to write out decision making steps. We are close!!!

Then add in the gapfilling step here!

#### Old way
For the food balance data there is a much greater variety of where a country's soyabean is directed for processing vs feed.
...
soy_feed_proportions <- soy_fb_cb_bean_cake_oil %>% 
...
soy_feed_proportions$prop_cake_in_processing[soy_feed_proportions$prop_cake_in_processing > 100] <- 1  

# check to see if cake_oil_production is close to 100% of processing per country
# not quite 100% (see comments above)
# soy_fb_cb_check <- soy_fb_cb_bean_cake_oil %>%
...
- calculate proportion soy DSQ going to feed via processing (multiply previous 2 proportions)
```{r}
soy_feed_proportions <- soy_feed_proportions %>% 
