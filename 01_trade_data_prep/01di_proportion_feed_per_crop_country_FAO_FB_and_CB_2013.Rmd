...
The above chunk shows that most domestic supply of soy cake is from feed with the lowest proportion for a country being 80%.
...
- "Soyabean Oil" is not typically used for feed so therefore it is not included
```{r}
...
Read in previously made `soy_feed_proporion` csv of percent feed for soy, add region column, and gapfill.
```{r}
...
## below is a data check to make sure that the above works as intended
