...
save_path <- "/home/shares/food-systems/social_justice_projects/food_trade_pressures/output_data/03b_gapfill_perc_feed_soy_palm_oilcrops_2013_CB_FB/"
```
...
```{r}
soy <- read_csv(paste0(feed_4a_path, "perc_dsq_soy_used_as_feed.csv"))
...
soy_1 <- soy
soy_1[152,1]                    # check to make sure this is Turkey
soy_1[152,1] <- "Turkey"
soy_1[37,1]                     # check to make sure this is Côte d'Ivoire
soy_1[37,1] <- "Côte d'Ivoire"
soy_1[33,1]                     # check to make sure this is Taiwan
soy_1[33,1] <- "Taiwan"
...
soy_2 <- soy_1 %>% 
  ...
  rename(soy_dsq = `Domestic supply quantity`, 
         soy_feed = Feed, 
...
# save csv
write_csv(soy_6, paste0(save_path, "perc_dsq_soy_used_as_feed_gapfilled.csv"))
```
...
```{r}
pk <- read_csv(paste0(feed_4a_path, "perc_dsq_palm_kernel_used_as_feed.csv"))

# have to rename Turkey because it got a ? symbol added when saved as csv previously
# can't use %in% or == to identify it because it isn't showing up as a string
pk_1 <- pk
pk_1[105,1]                    # check that this is Turkey
pk_1[105,1] <- "Turkey"
pk_1[27,1]                     # check that this is Côte d'Ivoire
pk_1[27,1] <- "Côte d'Ivoire"
pk_1[23,1]                     # check that this is Taiwan
pk_1[23,1] <- "Taiwan"
...
# save csv
write_csv(pk_10, paste0(save_path, "perc_dsq_palm_kernel_used_as_feed_gapfilled.csv"))
```
...
```{r}
oil <- read_csv(paste0(feed_4a_path, "perc_dsq_oilcrops_used_as_feed.csv"))

# have to rename Turkey because it got a ? symbol added when saved as csv previously
# can't use %in% or == to identify it because it isn't showing up as a string
oil_1 <- oil
oil_1[131,1]                    # make sure this is Turkey
oil_1[131,1] <- "Turkey"
oil_1[184,1]                    # make sure this is Côte d'Ivoire
oil_1[184,1] <- "Côte d'Ivoire"
oil_1[33,1]                     # make sure this is Taiwan
oil_1[33,1] <- "Taiwan"
...
# save csv
write_csv(oil_6, paste0(save_path, "perc_dsq_oilcrops_used_as_feed_gapfilled.csv"))
```
