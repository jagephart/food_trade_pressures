...
fao_trade_quantity_gapfill <- function(fao_trade_matrix_raw, year) {
  # subset columns to match Kastner data columns and subset to only 2017
...
Gapfill and save as csv for 2016, 2017, 2018 data
```{r}
# 2016 data
...
Average trade across three years
```{r}
...
# rbind trade df for three years of interest
average_trade <- rbind(trade_2016,
                       trade_2017,
                       trade_2018) %>%
  # group by all columns of specificity except for year
...
# NaN only occur in rows where the Value is NA
...
fao_production_crops_raw <- fread(paste0(fao_data_path, "Production_Crops_2017_E_All_Data_(Normalized)_7_20_22.csv"))
...
fao_production_crops_processed_raw <- fread(paste0(fao_data_path, "Production_CropsProcessed_2017_E_All_Data_(Normalized)_7_20_22.csv"))
