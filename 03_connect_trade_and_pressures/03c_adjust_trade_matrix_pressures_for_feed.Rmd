...
# remove all buffaloes meat and goats milk (not in trade matrix) and chickens_backyard_eggs&meat
...
We are adding "fw_aquaculture" to the beginning of the items to for freshwater aquaculture to make them easily distinguisable from marine fisheries and aquaculture.
```{r}
...
# tonnes_crop_feed_consumed <- tonnes_crop_feed_consumed %>% 
...
summary(crop_feed_animal_proportions) # no NAs
...
crop_feed_pressures <- crop_feed_pressures %>% 
  pivot_longer(cols = buffaloes_milk:fw_aquaculture_miscellaneous_diadromous_fishes,
...
Note: There is some trade and pressures data totals being changed by the proportion allocation. as most of the total change is <.1% we are going to assume for now that it is due to rounding error introduced from R and the proportions.
...
((sum(feed_crops_used_for_feed$tonnes_feed) - sum(crop_feed_pressures_adjusted$tonnes_traded_system_feed))/sum(feed_crops_used_for_feed$tonnes_feed))*100 # 0.61% = less than one percent, ignore this discrepancy

# water
sum(crop_feed_pressures_adjusted$water_traded_system_feed)
sum(feed_crops_used_for_feed$water_pressure_feed)
((sum(feed_crops_used_for_feed$water_pressure_feed) - sum(crop_feed_pressures_adjusted$water_traded_system_feed))/sum(feed_crops_used_for_feed$water_pressure_feed))*100 # 0.039%

# ghg
sum(crop_feed_pressures_adjusted$ghg_traded_system_feed)
sum(feed_crops_used_for_feed$ghg_pressure_feed)
((sum(feed_crops_used_for_feed$ghg_pressure_feed) - sum(crop_feed_pressures_adjusted$ghg_traded_system_feed))/sum(feed_crops_used_for_feed$ghg_pressure_feed))*100 # 0.062%

# nutrient
sum(crop_feed_pressures_adjusted$nutrient_traded_system_feed)
sum(feed_crops_used_for_feed$nutrient_pressure_feed)
((sum(feed_crops_used_for_feed$nutrient_pressure_feed) - sum(crop_feed_pressures_adjusted$nutrient_traded_system_feed))/sum(feed_crops_used_for_feed$nutrient_pressure_feed))*100 # 0.061%

# disturbance
sum(crop_feed_pressures_adjusted$disturbance_traded_system_feed)
sum(feed_crops_used_for_feed$disturbance_pressure_feed)
((sum(feed_crops_used_for_feed$disturbance_pressure_feed) - sum(crop_feed_pressures_adjusted$disturbance_traded_system_feed))/sum(feed_crops_used_for_feed$disturbance_pressure_feed))*100 # 0.13%
```
...
#### NOTE: ISO3c "Yem" has the most data that is different, not sure why this is happening and unsure how to check it.
...
## Stopped here, having trouble figuring out the best way to adjust feed pressure calculations for animals. TBD on path forward 
