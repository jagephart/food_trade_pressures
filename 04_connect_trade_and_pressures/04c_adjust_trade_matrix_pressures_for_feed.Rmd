...
animal_consumer_na <- animal_feed_pressures_trade %>% 
  filter(is.na(animal_consumer_iso3c))
```

Multiply traded feed pressures for each crop, animal system, and trading pair (feed producer and feed consumer) by the proportion of each animal traded between trading pair (animal producer and animal consumer). For the instances where we have no animal traded proportions we will make these zero.
```{r}
...
I think the next step is to separate the trade with no animal proportion data from the rest of the trade data and decide how to reintegrate it separately. Then we can follow the rest of the workflow as otherewise presented.
### Stopped here
