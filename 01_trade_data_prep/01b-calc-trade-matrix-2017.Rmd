...
- /home/shares/food-systems/social_justice_projects/food_trade_pressures/raw_data/Code
- /home/shares/food-systems/social_justice_projects/food_trade_pressures/ouput_data/01a_format_fao_trade
...
```{r}
filename <-paste("output_data/01b_calc_trade_matrix_2017/TradeMatrix_", prefer_import, "_", conversion_opt, "_", included_years, ".csv", sep="")
log.filename <- paste("output_data/01b_calc_trade_matrix_2017/logfile_TradeMatrix_", prefer_import, "_", conversion_opt, "_", included_years, ".txt", sep="")
```
...
Create a new folder in the directory (on the server) called "01b_calc_trade_matrix_2017".
This is where we'll save methods and the final trade matrix output.
```{r}
if(!dir.exists("output_data/01b_calc_trade_matrix_2017")) dir.create("output_data/01b_calc_trade_matrix_2017")
```
...
This data includes how much (tonnes) was traded between every country and product.
For example - tonnes of refined sugar traded between Algeria and Albania.
```{r}
### Trade data
...
# Remove first two columns - these no longer exist but may have in Kastner data
...
#test1 <- fread("raw_data/kastner_2022/CommodityBalances_LivestockFish_E_All_Data_(Normalized).csv")
...
data.import <- select(raw.trade.data, Reporter_Country_Code, Partner_Country_Code, Element_Code, Item_Code, Year, Value_average) %>% # () taken out for now because of average
...
Note this is orginal from the Schwarzmueller et al. 2022 script and parts are not being included in our data structure as we do not have sugar beet pressures available. These code chunks are commented out but remain here to maintain more of the original script. Th econversion from sugar cane and beet to sugar aggregate remains uncommented.
...
For the `matrix.operation` function to work properly the year values need to be single years or possible to be numerical values. Currently our data is "2015-2019" to denote that it is an average over that time span. The below code chunk will mutate the year column to just be 2017 so the rest of the operations can work as expected.
```{r}
primary.data <- primary.data %>% 
  # Year changed to 2017 for the sake of the calculations set up by Kastner
  mutate(Year = 2017)
production.all <- production.all %>% 
  # Year changed to 2017 for the sake of the calculations set up by Kastner
  mutate(Year = 2017)
...
q <- primary.data %>% distinct(Year, primary_item) # () removed because of average
...
setkey(primary.data, "primary_item", "Year") #() removed because of average
...
setkey(production.all, "Item_Code",  "Year", "Element_Code") # () removed because of average
...
```{r}
#### write file ####
fwrite(transformed.data, "output_data/01b_calc_trade_matrix_2017/trade_matrix_by_country_of_origin_2015_2019_import.csv")
```
...
This is all code I (Haley) wrote, none of this is from the Kastner paper.
