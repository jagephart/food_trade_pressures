...
trade_pressures_na <- crops_livestock_trade_pressures %>% 
  ...
  select(-c(nceas_group, pressure, pressure_per_tonne, food_group)) 

# how much of of the original data does this include?
# by quantity?
nrow(trade_pressures_na)/nrow(crop_livestock_trade_matrix)
# about 18% of the original rows of data

# by volume?
sum(trade_pressures_na$traded_tonnes)/sum(crop_livestock_trade_matrix$traded_tonnes)
# about 6% of total tonnes traded.
...
The old joining method seems to use SPAM_super more than the item code but I think it is simply a matter of missing pressure data for trade data. Could look into regional averages for crops that are traded but do not have pressures data. Or, could just ignore. 
